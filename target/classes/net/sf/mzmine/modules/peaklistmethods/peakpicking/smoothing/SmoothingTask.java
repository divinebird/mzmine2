...
public class SmoothingTask extends AbstractTask {
     ...
     */
    public SmoothingTask(final PeakList peakList, final ParameterSet smoothingParameters) {
    ...
    @Override public Object[] getCreatedObjects() {
                            ...
                            for (int i = 0; i < numScans; i++) {
                                ...
                                if (dataPoint != null && intensity > 0.0) {
                                    ...
                                    if (i != 0) {
                                        ...
                                        final double lastRT =
                                                dataFile.getScan(scanNumbers[i - 1]).getRetentionTime();
                            ...
                            if (!isCanceled() && maxScanNumber >= 0) {
                                ...
                                newRow.addPeak(dataFile,
                                               new SimpleFeature(
                                                       ...
                                                       new Range(peak.getRawDataPointsMZRange()),
                                                       intensityRange));
                            }
                        }
                    }
    ...
    private static double[] convolve(final double[] intensities, final double[] weights) {
        ...
        for (int i = 0;
            ...
            for (int j = Math.max(0, -k);
                 j < Math.min(fullWidth, numPoints - k);
                 j++) {
